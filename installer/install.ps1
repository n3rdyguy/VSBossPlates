...
.PARAMETER Mod
    Path to VSBossPlates.dll. Defaults to one found next to this script.
...
function Get-BepInEx {
    ...
    Info $url
    $temp = Join-Path ([System.IO.Path]::GetTempPath()) ("vsbp-bepinex-" + [guid]::NewGuid().ToString('N') + '.zip')
    try {
...
function Get-Mod {
    ...
    try {
        ...
        $zip = Join-Path ([System.IO.Path]::GetTempPath()) ("vsbp-mod-" + [guid]::NewGuid().ToString('N') + '.zip')
        ...
        try {
            $entry = $archive.Entries | Where-Object { $_.Name -eq 'VSBossPlates.dll' } | Select-Object -First 1
            if (-not $entry) { Fail 'The release zip did not contain VSBossPlates.dll.'; return $null }
            $out = Join-Path ([System.IO.Path]::GetTempPath()) ("vsbp-mod-" + [guid]::NewGuid().ToString('N') + '.dll')
...
if ($Interactive) {
    $modPath = Join-Path $Game 'BepInEx\plugins\VSBossPlates\VSBossPlates.dll'
...
if ($Uninstall) {
    ...
    $targets = @()
    $modDir = Join-Path $Game 'BepInEx\plugins\VSBossPlates'
    if (Test-Path $modDir) { $targets += $modDir }
    $cfg = Join-Path $Game 'BepInEx\config\com.n3rdyguy.vsbossplates.cfg'
    ...
    if ($All) {
        # BepInEx/plugins is shared, so removing the loader takes every other mod with it.
        # Refused rather than warned about: an uninstall of this mod silently uninstalling a
        # larger one is not a warning anybody deserves to have to read. -ForceBepInEx is the
        # escape hatch, because wanting the loader gone is legitimate.
        $plugins = Join-Path $Game 'BepInEx\plugins'
        if (Test-Path $plugins) {
            $others = Get-ChildItem $plugins -Force | Where-Object { $_.Name -ne 'VSBossPlates' }
            if ($others -and -not $ForceBepInEx) {
                ...
                Info 'Pass -ForceBepInEx to remove BepInEx and everything above with it.'
                Write-Host ''
                $All = $false
                $loader = @('BepInEx','dotnet','winhttp.dll','doorstop_config.ini','.doorstop_version','run_bepinex.sh','libdoorstop.so','libdoorstop.dylib')
                $targets = @($targets | Where-Object { $loader -notcontains (Split-Path $_ -Leaf) })
            }
            elseif ($others) {
...
if (-not $Mod) { $Mod = Find-Payload 'VSBossPlates.dll' }
if (-not $Mod -and -not $NoDownload) { $Mod = Get-Mod }
if (-not $Mod) {
    Fail 'Could not find or download VSBossPlates.dll.'
    Info "Download the release zip from $ReleasesPage then re-run with -Mod '<path to dll>'"
    Quit 6
}

$target = Join-Path $Game 'BepInEx\plugins\VSBossPlates'
New-Item -ItemType Directory -Path $target -Force | Out-Null
Step 'Installing the mod...'
Copy-Item $Mod (Join-Path $target 'VSBossPlates.dll') -Force
Ok "Mod installed: $target\VSBossPlates.dll"

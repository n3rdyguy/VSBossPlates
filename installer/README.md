...
| `--mod <dll>` | `VSBossPlates.dll`; defaults to one found beside the installer, else downloaded |
...
| `--force-bepinex` | With `--all`, remove BepInEx even when other mods are using it |
...
**`--all` will not remove BepInEx if other mods are installed.** BepInEx/plugins is shared, so
removing the loader takes every other mod with it, and this installer refuses rather than
warning: an uninstall of this mod silently uninstalling a larger one is not a warning anybody
deserves to have to read. The mod is removed and the loader is left alone. Pass
`--force-bepinex` if the loader really should go.

This is a deliberate departure from the installer this one was forked from, which warns and
proceeds. That was tolerable when one mod owned the loader; it is not now.
...
5. **Installs the mod** to `BepInEx/plugins/VSBossPlates/`.

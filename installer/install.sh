...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --force-bepinex) FORCE_BEPINEX=1; shift ;;
...
download_bepinex() {
  ...
  info "$url" >&2
  local temp
  temp="$(mktemp -t vsbp-bepinex-XXXXXX).zip"
...
download_mod() {
  ...
  step "Downloading VS Boss Plates${VERSION:+ $VERSION}..." >&2
  ...
  zip="$(mktemp -t vsbp-mod-XXXXXX).zip"
  ...
  out="$(mktemp -d -t vsbp-mod-XXXXXX)"
  if ! unzip -oqj "$zip" '*/VSBossPlates.dll' -d "$out" 2>/dev/null; then
    fail "The release zip did not contain VSBossPlates.dll." >&2
    rm -rf "$zip" "$out"; return 1
  fi
  rm -f "$zip"
  ok "Downloaded ${VERSION:-latest}" >&2
  printf '%s\n' "$out/VSBossPlates.dll"
}
...
if [ "$INTERACTIVE" -eq 1 ]; then
  ...
  [ -f "$GAME/BepInEx/plugins/VSBossPlates/VSBossPlates.dll" ] && hasmod="installed"
...
if [ "$UNINSTALL" -eq 1 ]; then
  ...
  TARGETS=()
  [ -d "$GAME/BepInEx/plugins/VSBossPlates" ] && TARGETS+=("$GAME/BepInEx/plugins/VSBossPlates")
  if [ "$KEEP_CONFIG" -eq 0 ] && [ -f "$GAME/BepInEx/config/com.n3rdyguy.vsbossplates.cfg" ]; then
    TARGETS+=("$GAME/BepInEx/config/com.n3rdyguy.vsbossplates.cfg")
  fi
  ...
  if [ "$REMOVE_ALL" -eq 1 ] && [ -d "$GAME/BepInEx/plugins" ]; then
    ...
    # larger one is not a warning anybody deserves to have to read. --force-bepinex is the
    # escape hatch, because wanting the loader gone is legitimate.
    others=$(find "$GAME/BepInEx/plugins" -maxdepth 1 -mindepth 1 ! -name 'VSBossPlates' 2>/dev/null || true)
    if [ -n "$others" ] && [ "$FORCE_BEPINEX" -eq 0 ]; then
...
'
      ...
      for t in "${TARGETS[@]}"; do
        case "$(basename "$t")" in
          BepInEx|dotnet|winhttp.dll|doorstop_config.ini|.doorstop_version|run_bepinex.sh|libdoorstop.so|libdoorstop.dylib) ;;
          *) KEPT+=("$t") ;;
        esac
      done
      TARGETS=("${KEPT[@]}")
...
[ -n "$MOD" ] || MOD=$(find_payload 'VSBossPlates.dll' || true)
...
TARGET="$GAME/BepInEx/plugins/VSBossPlates"
mkdir -p "$TARGET"
step "Installing the mod..."
cp -f "$MOD" "$TARGET/VSBossPlates.dll"

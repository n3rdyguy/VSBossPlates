...
namespace VSBossPlates.Installer;

/// <summary>
/// Installs BepInEx and VS Boss Plates into a Steam copy of Vampire Survivors.
///
...
internal static class Program
{
    ...
    private const string ModFolder = "VSBossPlates";
    private const string ModDll = "VSBossPlates.dll";
    ...
    private static int Uninstall(string game, string[] args)
    {
        ...
        if (targets.Count == 0)
        {
            Ok("Nothing to remove - no VS Boss Plates install found here.");
            return 0;
        }

        // Refusing rather than warning, and this is a deliberate departure from the installer
        // this one was forked from.
        //
        // That installer manages BepInEx and offers to remove it, which takes every other mod
        // with it. When one mod owned the loader that was merely blunt. Now that two mods
        // install through the same mechanism, an uninstall of the smaller one silently
        // uninstalling the larger one is not a warning anybody deserves to have to read - so
        // the mod is removed and BepInEx is left alone.
        //
        // --force-bepinex is there because "I really do want the loader gone" is a legitimate
        // thing to want, and the escape hatch costs one flag.
        if (all)
        {
            var others = OtherPlugins(game);
            if (others.Count > 0 && Array.IndexOf(args, "--force-bepinex") < 0)
            {
                Warn("Not removing BepInEx: other mods are installed and use it.");
                foreach (string o in others) Info("  - " + o);
                Console.WriteLine();
                Info("The mod itself will still be removed.");
                Info("Pass --force-bepinex to remove BepInEx and everything above with it.");
                Console.WriteLine();

                // Filtered by name against the same list the loader targets were built from,
                // rather than by spelling out paths again - two lists that have to agree are
                // one list too many.
                all = false;
                targets.RemoveAll(t => Array.IndexOf(LoaderArtifacts, Path.GetFileName(t)) >= 0);
            }
    ...
    private static string FindEpicGame()
    {
        ...
        foreach (string item in items)
        {
            ...
            // have to be hard-coded per storefront entry, which rots the same way a boss-name list
            // would. A launcher entry that is not this game simply will not contain the name.
    ...
    private static string DownloadBepInEx(string[] args, string game)
    {
        ...
        try
        {
            string temp = Path.Combine(Path.GetTempPath(), "vsbp-bepinex-" + Guid.NewGuid().ToString("N") + ".zip");
            using (var http = new System.Net.Http.HttpClient())
            {
                http.Timeout = TimeSpan.FromMinutes(5);
                http.DefaultRequestHeaders.Add("User-Agent", "VSBossPlates-Installer");
    ...
    /// <summary>
    /// Everything BepInEx itself puts in the game folder. One list, used both to build the
    /// removal set and to take it apart again when another mod turns out to need the loader.
    /// </summary>
    private static readonly string[] LoaderArtifacts =
    {
        "BepInEx", "dotnet",
    ...
    private const string Repo = "n3rdyguy/VSBossPlates";
    ...
    private static string DownloadMod(string[] args)
    {
        ...
        Step("Downloading VS Boss Plates" + (tag == null ? " (latest)" : " " + tag) + "...");
        ...
        try
        {
            ...
            string temp = Path.Combine(Path.GetTempPath(), "vsbp-mod-" + Guid.NewGuid().ToString("N") + ".zip");
            using (var response = http.GetAsync(url).GetAwaiter().GetResult())
            {
                response.EnsureSuccessStatusCode();
                using var stream = response.Content.ReadAsStream();
            ...
            string extracted = Path.Combine(Path.GetTempPath(), "vsbp-mod-" + Guid.NewGuid().ToString("N") + ".dll");
